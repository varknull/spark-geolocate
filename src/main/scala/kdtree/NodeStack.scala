...
/**
  ...
  **/
class NodeStack[A <: Node](dimension: Int, x: Double, y: Double) extends Stack[Node] with Serializable {
  ...
  def push(n: A): NodeStack[A] = {
    val nodeDistance = Utils.haversineDistance(x, n.x, y, n.y)
    ...
    sort
    worstDistance = Utils.haversineDistance(x, super.head.x, y, super.head.y)
    return this
  }
  ...
  private def sort() {
    for (i <- 1 until super.size) {
      ...
      val nextDistance = Utils.haversineDistance(x, next.x, y, next.y)
      val lastDistance = Utils.haversineDistance(x, last.x, y, last.y)
