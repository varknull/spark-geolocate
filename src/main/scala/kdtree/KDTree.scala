...
class KDTree extends Serializable {
  private var root: Node = null // root node of the tree
  private var k: Int = 0
  ...
  def insert(x: Double, y: Double, data: String) {
    if (root == null) {
      ...
      val (rover, dist) = bestAproximate(x, y) //We
  ...
  /**
    ...
    * */
  private def bestAproximate(pointX: Double, pointY: Double): (Node, Double) = {
    var rover = root
    var aprox: Node = null
    ...
    while (rover != null) {
      aprox = rover
      if (rover.switch) {
        rover = if (pointX >= rover.x) {
          rover.greater
        } else {
          rover.lesser
        }
      } else {
        rover = if (pointY >= rover.y) {
          rover.greater
        } else {
          rover.lesser
        }
     ...
     * node at "aprox". Then we compute the distance between the points
     * and returns a tuple containing both the node and the distance */
    (aprox, Utils.haversineDistance(pointX, aprox.x, pointY, aprox.y))
  }
  ...
  /* This method goes down the tree given a node. It checks if the best distance
